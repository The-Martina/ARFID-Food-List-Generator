// Image Exporter for ARFID Safe Foods List

/**
 * Captures and downloads the selected safe foods list as an image
 * @param {Array} selectedFoods - Array of selected food objects
 */
function generateAndDownloadImage(selectedFoods) {
    if (!selectedFoods || selectedFoods.length === 0) {
        alert('Please select at least one food to generate an image.');
        return;
    }
    
    // Create a container for the content to be captured
    const container = document.createElement('div');
    container.className = 'image-export-container';
    ...
    html2canvas(container, {
        ...
        link.download = 'All-My-Safe-Foods.png';
